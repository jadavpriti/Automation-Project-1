 ...
 */
 ...
 //The first test on 14.06.2023.
describe('This is first test suite, Priti Jadav', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[name="firstName"]').type('Priti')
        cy.get('input[name="lastName"]').type('Jadav')
        cy.get('input[name="password"]').type('Priti123')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        // Assert that phonenumber has tooltip with error message
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        cy.get('[name="password"]').clear().type(' ')
        cy.get('h2').contains('Password').click()
    ...
    it('User cannot add letters to phone number', () => {
        ...
        cy.get('#username').type('Priti11')
        cy.get('[data-testid="phoneNumberTestId"]').type('Crebrum Hub')
        cy.get('input[name="password"]').type('MyPass')
        cy.get('[name="confirm"]').type('MyPass')
